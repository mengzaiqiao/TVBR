...
class VBCAREngine(ModelEngine):
    ...
    def train_an_epoch(self, train_loader, epoch_id):
        ...
        for batch_id, sample in enumerate(train_loader):
            ...
            # n_neg = self.config["model"]["n_neg"]
            # pos_i = torch.cat([pos_i_1, pos_i_2])
            # neg_u = []
            # neg_i_1 = []
            # neg_i_2 = []
            # for i in range(len(sample)):
            #     neg_u_idxs = torch.randperm(len(sample))[:n_neg]
            #     neg_u.append(pos_u[neg_u_idxs])
            #     neg_i_1_idxs = torch.randperm(len(sample))[:n_neg]
            #     neg_i_2_idxs = torch.randperm(len(sample))[-n_neg:]
            #     neg_i_1.append(pos_i[neg_i_1_idxs])
            #     neg_i_2.append(pos_i[neg_i_2_idxs])
            # neg_u = torch.stack(neg_u)
            # neg_i_1 = torch.stack(neg_i_1)
            # neg_i_2 = torch.stack(neg_i_2)
